...
export const deleteIdea = async (id, setIdeas) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cette idée ?')) {
      try {
        const response = await fetch(`https://expressbai-production.up.railway.app/idees/${id}`, { method: 'DELETE' });
        if (response.ok) {
...
export const updateEvaluation = async (id, note, setIdeas) => {
    try {
      const response = await fetch(`https://expressbai-production.up.railway.app/idees/${id}/evaluation`, {
        method: 'POST',
...
export const updateStatus = async (id, statut, setIdeas) => {
  try {
      ...
      const response = await fetch(`https://expressbai-production.up.railway.app/idees/${id}/statut`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
